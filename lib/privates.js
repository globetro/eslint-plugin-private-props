...
const defaultPrivatePropertyMatchers = ['^_'];
const defaultPrivateMethodMatchers = ['^_', '^handle[A-Z]'];
...
var Privates = function(opts) {
  ...
  this.privatePropertyMatchers = compileRegExpStrings(
    opts.privatePropertyMatchers || defaultPrivatePropertyMatchers
  );

  this.privateMethodMatchers = compileRegExpStrings(
    opts.privateMethodMatchers || defaultPrivateMethodMatchers
  );
...
Privates.prototype = {
  ...
  _isPrivateProperty: function(name) {
    return _.some(this.privatePropertyMatchers, (prefix) => prefix.test(name));
  },

  _isPrivateMethod: function(name) {
    return _.some(this.privateMethodMatchers, (prefix) => prefix.test(name));
  }
};
